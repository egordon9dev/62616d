    ...
    if (SV_IN->key == SV_MAGIC) {
        ...
        if (temp & 2) {
            ...
            if (temp & 1) {
                ...
                SV_OUT->mode = 2;
                SV_OUT->outMode = 85;
                for (i = 0; i < 8; i++) SV_OUT->data[i] = svTeamName[i];
            }
        } else if (temp & 4)
            // Ready for mode set
            SV_OUT->mode = 8;
...
void _svNextByte() {
    uint8_t idx = svIndex;
    if (idx == 4) ioSetOutput(GPIOA, 11, 0);
...
void svInit() {
    ...
    for (i = 1; i < 16; i++) spiBufferTX[i] = (uint16_t)0x0000;
    SV_OUT->key = SV_MAGIC;
    SV_OUT->mode = 2;
    SV_OUT->flags = 0;
    SV_OUT->version = 1;
