...
double myCal[BOARD_NR_ADC_PINS];
...
int analogRead(unsigned char channel) {
    channel--;
    if (channel < BOARD_NR_ADC_PINS) return (int)adcRead(channel);
    return 0;
}
...
int analogReadCalibrated(unsigned char channel) {
    channel--;
    if (channel < BOARD_NR_ADC_PINS) return (int)adcRead(channel) - (int)(_analogState[channel].calibValue >> 4);
    return 0;
}
double myAnalogReadCalibrated(unsigned char channel) {
    channel--;
    if (channel < BOARD_NR_ADC_PINS) return adcRead(channel) - myCal[channel];
    return 0;
}
...
int analogReadCalibratedHR(unsigned char channel) {
    ...
    channel--;
    if (channel < BOARD_NR_ADC_PINS) return (int)(adcRead(channel) << 4) - (int)(_analogState[channel].calibValue);
    return 0;
}

// delay - taskDelay alias
void delay(const unsigned long time) { taskDelay((clock_t)time); }
...
void digitalWrite(unsigned char pin, bool value) {
    ...
    // undefined.
    if (pin < BOARD_NR_GPIO_PINS) ioSetOutput((GPIO_TypeDef *)_pinLookupTable[pin], (uint8_t)_pinIndexTable[pin], value);
}

// isAutonomous - Checks to see if the system is in autonomous mode
bool isAutonomous() { return (svFlags & SV_AUTONOMOUS) ? true : false; }

// isEnabled - Checks to see if the system is enabled
bool isEnabled() {
    uint16_t flags = svFlags;
    if (!(flags & SV_ENABLED)) {
        if (flags & SV_FMS) return false;
    }
    return true;
}

// isJoystickConnected - Checks to see if the specified joystick is plugged in
bool isJoystickConnected(unsigned char joystick) { return svIsJoystickConnected(joystick) ? true : false; }

// isOnline - Checks to see if the system is connected to a FMS/competition switch
bool isOnline() { return (svFlags & SV_FMS) ? true : false; }
...
unsigned long micros() { return (unsigned long)timeHighRes(); }

// millis - Arduino-compatible alias for timeLowRes()
unsigned long millis() { return (unsigned long)timeLowRes(); }
...
void motorStop(unsigned char channel) { motorControlSet(channel, 127); }
...
void pinMode(unsigned char pin, unsigned char mode) {
    if (pin < BOARD_NR_GPIO_PINS) ioSetDirection((GPIO_TypeDef *)_pinLookupTable[pin], (uint8_t)_pinIndexTable[pin], mode);
}

// powerLevelBackup - Get backup battery voltage in millivolts, or 0 if not connected
unsigned int powerLevelBackup() { return svGetBackupBattery(); }

// powerLevelMain - Get main battery voltage in millivolts, or 0 if not connected
unsigned int powerLevelMain() { return svGetMainBattery(); }

// setTeamName - Sets the team name of the Cortex
void setTeamName(const char *name) { svSetTeamName(name); }
...
TaskHandle taskRunLoop(void (*fn)(void), const unsigned long increment) {
    ...
    return taskCreate(_taskRunLoopHelper, TASK_DEFAULT_STACK_SIZE, (void *)lt, TASK_PRIORITY_DEFAULT + 1);
}

// wait - taskDelay alias
void wait(const unsigned long time) { taskDelay((clock_t)time); }

// waitUntil - taskDelayUntil alias
void waitUntil(unsigned long *previousWakeTime, unsigned long time) { taskDelayUntil((clock_t *)previousWakeTime, (clock_t)time); }
