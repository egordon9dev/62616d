...
#include "pros_src/FreeRTOS.h"
#include "pros_src/kernel.h"
#include "pros_src/periph.h"

// Status flags for the flag variable
// Set if and only if ping process is running
#define ULTRA_PINGING 0x1
// Set if and only if ping process is waiting for a ping to return; false if
// simply waiting out the pulse duration
...
// ultrasonicInit - Initializes an ultrasonic sensor on the specified digital
// ports portEcho is the orange cable and portPing is the yellow cable
...
void myUltrasonicShutdown(Ultrasonic ult) {
  ...
  if (ultra) {
    _enterCritical();
    {
      ...
      // Find our previous sensor (who links to us) and set its portTop to skip
      // us
