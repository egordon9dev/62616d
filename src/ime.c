...
static bool imeSetAddress(uint8_t currentAddress, uint8_t newAddress) { return i2cWriteRegister(currentAddress, IME_REG_SET_ADDR, newAddress); }
...
static void imeLoop(void *cnt) {
    ...
    while (imeTask == NULL) taskDelay(10);
...
unsigned int imeInitializeAll() {
    uint8_t newAddress = IME_ADDR_MIN;
    unsigned int encodersFound;
    // Clear out status array
    for (encodersFound = 0; encodersFound < IME_MAX_NUM; encodersFound++) ime[encodersFound].flags = (uint16_t)0;
    ...
    if (imeTask == NULL) {
        ...
        TaskHandle h = taskCreate(imeLoop, TASK_MINIMAL_STACK_SIZE, (void *)encodersFound, TASK_PRIORITY_HIGHEST - 1);
        if (!h) _exit(STATUS_CANT_START_SYSTEM);
        imeTask = h;
    }
