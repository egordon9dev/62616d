...
void auton2(bool leftSide, int stackH, int zone) {
    unsigned long t0 = millis(), prevT = millis(), funcT0 = millis();
        ...
        while (allowRepeat) {
            ...
            if (i == j++) {  // deploy
                ...
                if (zone == 20) {
                    ...
                    setDL(-127);
                    //****************************************************************************************************
                    if (d > -3) {  // <----------- ***INSERT ULTRASOUND DETECTION HERE TO DETECT WHEN MGL IS UNLOADED***
                        setMGL(0);
                    } else {
                        setMGL(-20);
                    }
